 ...
 global $wpdb;
	...
	foreach($reponse as $donnes)
	{
	 if ($_GET['use'] == 'sent' || $_GET['use'] == 'draft')
		{
		?>
		<tr><td><?php echo get_user_by('id', $donnes->receiver)->display_name; ?></td><td><a href="#" class="messagerie_read_draft_sent" id_message="<?php echo $donnes->id; ?>"><?php echo esc_html($donnes->objet); ?></a></td><td><?php echo wp_kses(substr($donnes->message, 0, 20), array('')); ?></td><td><?php echo $donnes->date_envoi; ?></td><?php if ($_GET['use'] == 'draft') { echo '<td><a class="messagerie_delete_draft" href="#" id_message="' . $donnes->id . '"><img alt="Del" src="' .  plugins_url()  .'/messagerie/delete.png" style="width: 25px; height: 25px;"/></a></td>'; }?></tr>
<?php 
		}
		else 
		{ 
		?>
				<tr <?php if ($donnes->unread == 1) { echo 'style="font-weight: bold;"'; }?>><td><?php if ($donnes->unread == 1) { echo '<img style="width: 50%" src="' . plugins_url() . '/messagerie/etoile.png"/>'; }?></td><td><?php echo get_user_by('id', $donnes->sender)->display_name; ?></td><td><a href="#" <?php if ($_GET['use'] == 'trash') {echo ' trash="1" '; } ?>class="messagerie_read_draft_sent" id_message="<?php echo $donnes->id; ?>"><?php echo $donnes->objet; ?></a></td><td><?php echo wp_kses(substr($donnes->message, 0, 20), array('')); ?></td><td><?php echo $donnes->date_envoi; ?></td><td><a href="#" class="<?php if ($_GET['use'] == 'trash') { echo 'messagerie_undo'; } else { echo 'messagerie_delete'; }?>" id_message="<?php echo $donnes->id; ?>"><img alt="<?php if ($_GET['use'] == 'trash') { echo 'Undo';} else { echo 'Del';}?>" style="width: 25px; height: 25px;" src="<?php echo plugins_url(); ?>/messagerie/<?php if ($_GET['use'] == 'trash') { echo 'undo.png';} else { echo 'delete.png';}?>"/></a></td></tr>
		<?php 
		}
	}
